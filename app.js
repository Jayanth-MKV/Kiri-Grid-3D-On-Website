...
function init(mod) {
  ...
  function find_refs(cache, path) {
    ...
    let lines = fs.readFileSync(full).toString().split("\n");
  ...
  // load modules
  lastmod(`${dir}/mod`) &&
    fs.readdirSync(`${dir}/mod`).forEach((dir) => {
...
function initModule(mod, file, dir) {
  ...
  require_fresh(file)({
    ...
    path: {
      ...
      map: (arg) => {
        mod.add(fixedmap(arg));
      },
      full: (arg) => {
        mod.add(fullpath(arg));
      },
      static: (root, pre) => {
        mod.static(pre || "/", root);
      },
...
const script = {
  kiri: ["@devices", "@icons", "kiri/ui", "&main/kiri", "&kiri/lang-en"],
  kiri_work: ["kiri-run/worker", "&main/kiri"],
  kiri_pool: ["&kiri-run/minion", "&main/kiri"],
  engine: ["&kiri-run/engine", "&main/kiri"],
  frame: ["kiri-run/frame"],
  meta: ["main/meta"],
  mesh: ["&main/mesh"],
  mesh_work: ["&mesh/work"],
  mesh_pool: ["&mesh/pool"],
  cache: ["moto/license", "main/service"],
  service: ["moto/license", "moto/service"],
};
...
function handleVersion(req, res, next) {
  ...
  if (
    ["/kiri/", "/mesh/", "/meta/"].indexOf(req.app.path) >= 0 &&
    req.url.indexOf(vstr) < 0
  ) {
...
function concatCode(array) {
  ...
  if (debug) {
    const code = [
      oversion
        ? `self.debug_version='${oversion}';self.enable_service=${serviceWorker};`
        : "",
      "self.debug=true;",
      "(function() { let load = [ ",
    ];
...
function getCachedFile(file, fn) {
  ...
  let cachePath =
      cacheDir +
      "/" +
      file.replace(/\//g, "_").replace(/\\/g, "_").replace(/:/g, "_"),
